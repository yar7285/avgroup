 <div class="widget-first widget recent-posts">
        <h3>{{ Lang::get('ru.latest_portfolios') }}</h3>
            ...
            @if(!$portfolios->isEmpty())
                @foreach($portfolios as $portfolio)
                    <div class="hentry-post group">
                        ...
                        <div class="text">
                            <a href="{{ route('services.show',['alias' => $portfolio->alias]) }}" title="{{ $portfolio->title }}" class="title">{{ $portfolio->title }}</a>
                            <p>{!! str_limit($portfolio->text,130) !!}</p>
                            <a class="read-more" href="{{ route('services.show',['alias' => $portfolio->alias]) }}">&rarr;{{ Lang::get('ru.read_more') }}</a>
                        </div>
                    </div>
                @endforeach
            @endif
            ...
            @foreach($comments as $comment)
                <div class="the-post group">
                    <div class="avatar">
                        ...
                        <img alt="" src="https://www.gravatar.com/avatar/{{ $hash }}?d=mm&s55" class="avatar" />
                    </div>
                    <span class="author"><strong><a href="#">{{ isset($comment->usrer) ? $comment->user->name : $comment -> name }}</a></strong> in</span>
