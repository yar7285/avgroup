                    ...
                    <div class="hentry work group portfolio-sticky portfolio-full-description">
                        <div class="work-thumbnail">
                            ...
                            <div class="work-overlay">
                                <h3><a href="{{route('services.show',['alias'=>$item->alias])}}">{{$item->title}}</a></h3>
                        ...
                        <div class="work-description">
                            <h2><a href="{{route('services.show',['alias'=>$item->alias])}}">{{$item->title}}</a></h2>
                            <p class="work-categories">in: <a href="#">{{$item->filter->title}}</a></p>
                                <p>{{str_limit($item->text,200)}}</p>
                             <a href="{{route('services.show',['alias'=>$item->alias])}}" class="read-more">Читати далі</a>
                        </div>
                    </div>
                    ...
                    <div class="overlay_a related_img">
                        <div class="overlay_wrapper">
                            ...
                            <div class="overlay">
                                ...
                                <a class="overlay_project" href="{{route('services.show',['alias'=>$item->alias])}}"></a>
                                <span class="overlay_title">{{$item->title}}</span>
                            </div>
                        </div>
                    </div>
                    <h4><a href="{{route('services.show',['alias'=>$item->alias])}}">{{$item->title}}</a></h4>
                    <p>{{str_limit($item->text,200)}}</p>
                </div>
                @endforeach
            </div>
